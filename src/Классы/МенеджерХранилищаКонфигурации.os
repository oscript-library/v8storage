...
Процедура СоздатьХранилищеКонфигурации(Знач ПодключитьБазуКхранилищу = Ложь,
                                        Знач ПравилоПоддержкиДляРазрешенных = Неопределено,
                                        Знач ПравилоПоддержкиДляЗапрешенных = Неопределено) Экспорт

    Параметры = СтандартныеПараметрыЗапуска();

    Параметры.Добавить("/ConfigurationRepositoryCreate ");

    Если Не (ПравилоПоддержкиДляРазрешенных = Неопределено
        И ПравилоПоддержкиДляЗапрешенных = Неопределено) Тогда

        Параметры.Добавить("-AllowConfigurationChanges");

        Если ПравилоПоддержкиДляРазрешенных = 0 Тогда
        ...
        Если ПравилоПоддержкиДляЗапрешенных = 0 Тогда
    ...
    Если ПодключитьБазуКхранилищу Тогда
...
Процедура ПрочитатьХранилище(Знач НомерНачальнойВерсии = 1) Экспорт
    ...
    ВременныеФайлы.УдалитьФайл(ПутьКФайлуОтчетаJSON);
...
//
// Параметры:
//   ПутьКФайлуОтчета - Строка - путь к файлу отчета в формате mxl
...
//
// Параметры:
//   ПапкаИсходников - Строка - путь к каталогу исходников обработки
//   ИмяФайлаОбъекта - Строка - Путь к будущему файлу обработки
//
